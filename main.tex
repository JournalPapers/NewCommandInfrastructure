...
\section*{UML Class Diagram Example (TikZ)}

\tikzset{
  class/.style={rectangle, draw=black, fill=white!10, text width=8em, text centered, rounded corners, minimum height=3em},
  assoc/.style={-latex, thick}
}

\begin{tikzpicture}[node distance=4cm, every node/.style={class}]
% Nodes
\node (Student) {Student\\- studentId: int\\- name: String\\+ enroll()\\+ drop()};
\node (Course) [right=of Student] {Course\\- courseId: int\\- title: String\\+ addStudent()\\+ removeStudent()};
\node (Professor) [below=of Student, xshift=2cm] {Professor\\- profId: int\\- name: String\\+ assignCourse()};

% Associations
\draw[assoc] (Student.east) -- (Course.west) node[midway, above]{enrolls};
\draw[assoc] (Professor.north) -- (Course.south) node[midway, right]{teaches};
...
\begin{tikzpicture}
      \begin{class}[text width=8cm]{command `<<component>>`}{0,0}
